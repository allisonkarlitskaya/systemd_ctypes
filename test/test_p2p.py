...
class CommonTests:
    ...
    def test_method_throws(self):
        async def test():
            with pytest.raises(BusError, match=r'cockpit.Error.ZeroDivisionError: Divide by zero'):
                await self.client.call_method_async(None, '/test', 'cockpit.Test', 'Divide', 'ii', 1554, 0)
        run_async(test())

    def test_method_throws_oserror(self):
        async def test():
            with pytest.raises(BusError, match=r'org.freedesktop.DBus.Error.FileNotFound: .*notthere.*'):
    ...
    def test_unexport(self):
        async def test():
            ...
            with pytest.raises(BusError, match=r"org.freedesktop.DBus.Error.UnknownObject: Unknown object '/test'."):
